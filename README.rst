...
Then, the EH generates enclave quote and verifies it with IAS. Enclave's public key hash is
...
the same value. ``MR_SIGNER`` is the enclave signing key hash. They can both be verified by
voters from the IAS report which is shown later.
::
...
The EH runs the hosting application in interactive mode::
...
The ``-o`` option allows accepting quotes from not fully up-to-date platforms (missing BIOS or
firmware updates). The ``-E`` option verifies the enclave SGX hash (``MR_ENCLAVE``) which ensures
that the enclave source/binary is unmodified from what is expected. The ``-n`` option checks for
nonce provided during IAS request (done by the EH), this may not be needed.
