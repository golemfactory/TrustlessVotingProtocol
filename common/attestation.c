...
int verify_ias_report(const uint8_t* ias_report, size_t ias_report_size, uint8_t* ias_sig_b64,
    ...
    ret = mbedtls_base64_decode(NULL, 0, &ias_sig_size, ias_sig_b64, ias_sig_b64_size);
    ...
    size_t quote_size = 0;
    ret = mbedtls_base64_decode(NULL, 0, &quote_size, (const unsigned char*)node->valuestring,
    ...
    ret = mbedtls_base64_decode(report_quote, quote_size, &quote_size,
                                (const unsigned char*)node->valuestring, strlen(node->valuestring));
    if (ret != 0) {
        ERROR("IAS report: failed to decode report quote\n");
        goto out;
    }
