...
int ias_get_sigrl(struct ias_context_t* context, uint8_t gid[4], size_t* sigrl_size, void** sigrl) {
    ...
    *sigrl_size = 0;
    if (ias_resp->data) {
        ret = mbedtls_base64_decode(NULL, 0, sigrl_size, (const unsigned char*)ias_resp->data,
                                    strlen(ias_resp->data));
        if (ret != MBEDTLS_ERR_BASE64_BUFFER_TOO_SMALL) {
            ERROR("Failed to base64-decode SigRL: %d\n", ret);
            goto out;
        }
        ...
        *sigrl = malloc(*sigrl_size);
        ...
        ret = mbedtls_base64_decode(*sigrl, *sigrl_size, sigrl_size,
                                    (const unsigned char*)ias_resp->data, strlen(ias_resp->data));
        if (ret != 0) {
            ERROR("Failed to base64-decode SigRL: %d\n", ret);
            goto out;
        }
...
static int ias_send_request(struct ias_context_t* context, struct ias_request_resp* ias_resp,
    ...
    ret = mbedtls_base64_encode(NULL, 0, &quote_b64_size, quote, quote_size);
    if (ret != MBEDTLS_ERR_BASE64_BUFFER_TOO_SMALL) {
        ERROR("Failed to base64 encode the quote: %d\n", ret);
        goto out;
    }

    quote_b64 = malloc(quote_b64_size);
    ret = mbedtls_base64_encode((unsigned char*)quote_b64, quote_b64_size, &quote_b64_size, quote,
                                quote_size);
    if (ret != 0) {
        ERROR("Failed to base64 encode the quote: %d\n", ret);
        goto out;
...
int ias_verify_quote(struct ias_context_t* context, const void* quote, size_t quote_size,
    ...
    if (report_path) {
        ret = write_file(report_path, ias_resp.data, ias_resp.data_size);
        if (ret != 0) {
    ...
    if (sig_path) {
        ret = write_file(sig_path, ias_resp.signature, ias_resp.signature_size);
        if (ret != 0) {
    ...
    if (cert_path) {
        ...
        ret = write_file(cert_path, ias_resp.certificate, ias_resp.certificate_size);
        if (ret != 0) {
    ...
    if (advisory_path) {
        ...
        if (ias_resp.advisory_url_size > 0) {
            ret = append_file(advisory_path, ias_resp.advisory_url, ias_resp.advisory_url_size);
            if (ret != 0) {
        ...
        if (ias_resp.advisory_ids_size > 0) {
            ret = append_file(advisory_path, ias_resp.advisory_ids, ias_resp.advisory_ids_size);
            if (ret != 0) {
